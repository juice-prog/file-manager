...
namespace mini_total_commander
{
    public partial class MiniTCPanel : UserControl//, IPanel
    {
        ...
        {
            get {
                if (поле.SelectedItem != null)
                {
                    if (поле.SelectedItem.ToString().StartsWith("<D>"))    // to remove symbol <D> from the selected item
                                                                              //to avoid lack of backslash in the path (with root dir)
                    {
                        if (!поле.SelectedItem.ToString().Contains("\\")) return "\\" + поле.SelectedItem.ToString().Remove(0, 3);

                        else return поле.SelectedItem.ToString().Remove(0, 3);
                    }
                    else if (!поле.SelectedItem.ToString().Contains("\\")) return "\\" + поле.SelectedItem.ToString();
                    else return поле.SelectedItem.ToString();
                }
                else return null;
